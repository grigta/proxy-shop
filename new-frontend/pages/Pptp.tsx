...
const Pptp: React.FC = () => {
  ...
  const {
    ...
    purchaseFunction: async (proxy, couponCode) => {
      return await apiClient.purchasePptp({
        country: proxy.country,
        state: proxy.state || undefined,
        city: proxy.city || undefined,
        zip_code: proxy.zip || undefined,
  ...
  useEffect(() => {
    const loadCountries = async () => {
      ...
      try {
        const data = await apiClient.getCountries(ProxyType.PPTP);
        setCountries(data);
  ...
  useEffect(() => {
    ...
    const loadProxies = async () => {
      ...
      try {
        const response = await apiClient.getPptpProducts({
          country: selectedCountry,
          state: selectedState || undefined,
          city: appliedSearchKeyword || undefined,
        ...
        setProductsError(err.response?.data?.detail || 'Failed to load servers');
        setProxies([]);
      } finally {
        setLoading(false);
      }
    ...
    loadProxies();
  }, [selectedCountry, selectedState, appliedSearchKeyword, selectedZip, page, pageSize]);

  // Handle search button click
  const handleSearch = () => {
    // Apply draft values to applied filters
      ...
      <div className="bg-white dark:bg-[#18181b] p-4 rounded-xl border border-gray-200 dark:border-[#27272a] shadow-sm mb-6 transition-colors duration-200">
        <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 grid grid-cols-2 md:grid-cols-5 gap-4 w-full">
                ...
                <div>
                    <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 block">Region</label>
                    <input 
                      type="text" 
                      placeholder="WA" 
                      value={draftState}
                ...
                <div className="col-span-2 md:col-span-2">
                     <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 block">City/ISP Keyword</label>
                     <input 
                       type="text" 
                       placeholder="Search by City or ISP" 
      ...
      <div className="bg-white dark:bg-[#18181b] border border-gray-200 dark:border-[#27272a] rounded-xl shadow-sm overflow-hidden transition-colors duration-200">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 dark:text-gray-400 uppercase bg-gray-50 dark:bg-[#27272a] border-b border-gray-200 dark:border-[#3f3f46]">
              <tr>
                <th className="px-6 py-3 font-medium">Server IP</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-[#27272a]">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                    {productsError ? 'Failed to load servers. Please try again.' : 'No servers found matching your filters.'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-[#27272a] bg-gray-50 dark:bg-[#18181b] flex justify-between items-center transition-colors">
            <span className="text-xs text-gray-500 dark:text-gray-400">
              Показано {processedProxies.length} серверов на странице {page} (всего доступно {total} без учёта фильтра скорости)
            </span>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#18181b] border border-gray-200 dark:border-[#27272a] rounded-xl shadow-2xl max-w-md w-full mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Confirm PPTP Purchase</h2>
              <button 
                onClick={closeModal}
            ...
            <div className="space-y-4">
              ...
              {/* Warning about server allocation */}
              <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                <p className="text-xs text-blue-800 dark:text-blue-300">
                  Note: PPTP server will be allocated based on availability in the selected location.
                </p>
              </div>
      ...
      {modalState === 'success' && purchaseResponse && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#18181b] border border-gray-200 dark:border-[#27272a] rounded-xl shadow-2xl max-w-md w-full mx-4 p-6">
            ...
            <h2 className="text-xl font-bold text-center text-gray-900 dark:text-white mb-2">PPTP Server Purchased!</h2>
            ...
            <div className="space-y-4">
              ...
              {purchaseResponse.proxies && purchaseResponse.proxies.length > 0 && (
                <div className="bg-gray-50 dark:bg-[#27272a] rounded-lg p-4 space-y-3">
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Server Details</h3>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button
                        onClick={() => handleCopy('ip', purchaseResponse.proxies[0].ip)}
                    ...
                    <div className="font-mono text-sm text-gray-900 dark:text-white bg-white dark:bg-[#18181b] rounded px-2 py-1 border border-gray-200 dark:border-[#3f3f46]">
                      {purchaseResponse.proxies[0].ip}
                    </div>
                  </div>

                  {/* Port (if provided) */}
                  {purchaseResponse.proxies[0].port && (
                    <div>
                      <div className="flex justify-between items-center mb-1">
                  ...
                  {purchaseResponse.proxies[0].login && (
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <button
                          onClick={() => handleCopy('username', purchaseResponse.proxies[0].login)}
                          className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1"
                        >
                  ...
                  {/* Location details */}
                  {(purchaseResponse.proxies[0].country || purchaseResponse.proxies[0].state || purchaseResponse.proxies[0].city) && (
                    <div className="pt-2 border-t border-gray-200 dark:border-[#3f3f46]">
                      <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
                        {purchaseResponse.proxies[0].country && (
                          <div>Country: {purchaseResponse.proxies[0].country}</div>
                        )}
                        {purchaseResponse.proxies[0].state && (
                          <div>Region: {purchaseResponse.proxies[0].state}</div>
                        )}
                        {purchaseResponse.proxies[0].city && (
                          <div>City: {purchaseResponse.proxies[0].city}</div>
                        )}
                      </div>
                    </div>
                  )}
